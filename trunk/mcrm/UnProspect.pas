...
Type TRBFuncoesProspect = class(TRBLocalizaProspect)
  ...
  public
    ...
    function CadastraEmailEmailSuspect(VpaDSuspect : TRBDProspect;VpaDesEmail : String) : string;
end;
...
{******************************************************************************}
function TRBFuncoesProspect.CadastraEmailEmailSuspect(VpaDSuspect : TRBDProspect;VpaDesEmail : String) : string;
var
...
begin
  ...
  begin
    if not VpfFunEmarketing.ExisteEmail(VpaDesEmail) then
      result := AdicionaEmailContatoProspect(VpaDSuspect.CodProspect,VpaDesEmail,'');
    VpaDesEmail := DeleteAteChar(VpaDesEmail,'@');
  end;
  if UpperCase(copy(VpaDesEmail,1,4)) = 'WWW.' then
    VpaDesEmail := copy(VpaDesEmail,5,length(VpaDesEmail) - 4);
  if result = '' then
  begin
    if not VpfFunEmarketing.EmailDominioProvedores(VpaDesEmail) then
    begin
      VpfFunEmarketing.CadastraEmailsAdicionaisProspect(VpaDSuspect.CodProspect, VpaDesEmail);
    end;
  end;
