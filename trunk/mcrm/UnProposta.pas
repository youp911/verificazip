...
Type TRBFuncoesProposta = class(TRBLocalizaProposta)
  private
    ...
    procedure ExcluiPropostaProduto(VpaDProposta : TRBDPropostaCorpo);
    procedure ExcluiPropostaProdutoSemQtd(VpaDProposta : TRBDPropostaCorpo);
    procedure ExcluiPropostaAmostra(VpaDProposta: TRBDPropostaCorpo);
    procedure ExcluiPropostaLocacao(VpaDProposta: TRBDPropostaCorpo);
    procedure ExcluiPropostaServico(VpaDProposta: TRBDPropostaCorpo);
    procedure ExcluiLocacaoFranquia(VpaDProposta: TRBDPropostaCorpo);
    procedure ExcluiPropostaProdutoRotuloado(VpaDProposta: TRBDPropostaCorpo);
    procedure ExcluiPropostaVendaAdicional(VpaDProposta: TRBDPropostaCorpo);
  ...
  public
    ...
    function EnviaEmailCliente(VpaDProposta : TRBDPropostaCorpo;VpaDCliente : TRBDCliente) : string;
    function AlteraEstagioProposta(VpaCodFilial,VpaSeqProposta, VpaCodUsuario, VpaCodEstagio : Integer;VpaDesMotivo : String):String;
...
Uses FunSql,Constantes, FunArquivos, FunString, fundata, FunObjeto;
...
{******************************************************************************}
procedure TRBFuncoesProposta.ExcluiPropostaProduto(VpaDProposta : TRBDPropostaCorpo);
begin
  if (VpaDProposta.SeqProposta <> 0)then
  begin
...
{******************************************************************************}
procedure TRBFuncoesProposta.ExcluiPropostaProdutoSemQtd(VpaDProposta : TRBDPropostaCorpo);
begin
  if (VpaDProposta.SeqProposta <> 0)then
  begin
...
{******************************************************************************}
procedure TRBFuncoesProposta.ExcluiPropostaAmostra(VpaDProposta: TRBDPropostaCorpo);
begin
  if VpaDProposta.SeqProposta <> 0 then
  begin
...
{******************************************************************************}
procedure TRBFuncoesProposta.ExcluiPropostaLocacao(VpaDProposta: TRBDPropostaCorpo);
begin
  ExcluiLocacaoFranquia(VpaDProposta);
...
{******************************************************************************}
procedure TRBFuncoesProposta.ExcluiPropostaServico(VpaDProposta: TRBDPropostaCorpo);
begin
...
{******************************************************************************}
procedure TRBFuncoesProposta.ExcluiLocacaoFranquia(VpaDProposta: TRBDPropostaCorpo);
begin
...
{******************************************************************************}
procedure TRBFuncoesProposta.ExcluiPropostaProdutoRotuloado(VpaDProposta: TRBDPropostaCorpo);
begin
...
{******************************************************************************}
procedure TRBFuncoesProposta.ExcluiPropostaVendaAdicional(VpaDProposta: TRBDPropostaCorpo);
begin
...
begin
  AdicionaSQLAbreTabela(Tabela,'Select PRP.SEQPRODUTO, PRP.CODPRODUTO, PRP.CODCOR, PRP.NOMCOR, PRP.NOMPRODUTO, ' +
                               ' PRP.QTDPRODUTO, PRP.VALUNITARIO,PRP.VALTOTAL, PRP.DESUM, PRP.NUMOPCAO, ' +
...
begin
  ...
  AdicionaSQLAbreTabela(Tabela,'Select PPR.SeqItem, PPR.SeqProduto, PPR.CodFormatoFrasco, PPR.CodMaterialFrasco, '+
                       ' PPR.AltFrasco, PPR.LarFrasco, PPR.ProFrasco, PPR.DiaFrasco, PPR.LarRotulo, '+
...
begin
  ...
  begin
    ...
    try
      ...
      on e : exception do
      begin
        result := 'ERRO NA GRAVAÇÃO DOS PRODUTOS DA PROPOSTA!!!'+#13+e.message;
        break;
      end;
    end;
  end;
...
begin
  ...
  begin
    ...
    try
      ...
      on E:Exception do
      begin
        Result:= 'ERRO NA GRAVAÇÃO DOS SERVIÇOS DA PROPOSTA!!!'#13+E.Message;
        Break;
      end;
    end;
  end;
...
begin
  ...
  try
    ...
    on e : exception do result := 'ERRO NA GRAVAÇÃO DO LOG DO ESTAGIO DA PROPOSTA!!!'#13+e.message;
  end;
end;
...
begin
  ...
  try
    ...
    on E:Exception do
      Result:= 'ERRO NA GRAVAÇÃO DO EMAIL DA PROPOSTA!!!'#13+E.Message;
  end;
end;
...
begin
  result := '';
  ExcluiPropostaProduto(VpaDProposta);
  ExcluiPropostaProdutoSemQtd(VpaDProposta);
  ExcluiPropostaAmostra(VpaDProposta);
  ExcluiPropostaLocacao(VpaDProposta);
  ExcluiPropostaServico(VpaDProposta);
  ExcluiPropostaProdutoRotuloado(VpaDProposta);
  ExcluiPropostaVendaAdicional(VpaDProposta);
  ...
  begin
    ...
    begin
      ...
      if LanOrcamento <> 0 then
        Cadastro.FieldByname('LANORCAMENTO').AsInteger := LanOrcamento
      else
      ...
      try
        ...
        on e : exception do result := 'ERRO NA GRAVAÇÃO DA PROPOSTA!!!'#13+e.message;
      end;
    end;
  end;
  Cadastro.close;
...
{******************************************************************************}
function TRBFuncoesProposta.EnviaEmailCliente(VpaDProposta : TRBDPropostaCorpo;VpaDCliente : TRBDCliente) : string;
var
...
begin
  ...
  Sistema.CarDFilial(VpfDFilial,VpaDProposta.CodFilial);
  result := '';
  if VpaDProposta.DesEmail = '' then
    result := 'E-MAIL DO PROSPECT NÃO PREENCHIDO!!!'#13'Falta preencher o e-mail do prospect.';
  if result = '' then
  begin
    VpfOpcoes := CarOpcoesProposta(VpaDProposta);
    ...
    begin
      ...
      case VpaDProposta.TipModeloProposta of
        0 : MontaEmailProposta(VpfEmailHTML.Body,VpaDProposta,VpaDCliente,VpfDFilial,VpfOpcao);
        1 : MontaEmailPropostaModelo1(VpfEmailHTML.Body,VpaDProposta,VpaDCliente,VpfDFilial,VpfOpcao);
        2 : MontaEmailPropostaModelo2(VpfEmailHTML.Body,VpaDProposta,VpaDCliente,VpfDFilial);
        3 : MontaEmailPropostaModelo3(VpfEmailHTML.Body,VpaDProposta,VpaDCliente,VpfDFilial);
        4 : MontaEmailsPropostaHomero(VpfEmailHTML.Body,VpaDProposta,VpaDCliente,VpfDFilial);
        5 : begin
              MontaEmailPropostaRotuladora(VpfEmailHTML.Body,VpaDProposta,VpaDCliente,VpfDFilial);
//              exit;
            end;
      end;
...
//      VpfEmailHTML.Body.SaveToFile('c:\proposta.html');
      ...
      begin
        VprMensagem.ReceiptRecipient.Text  := VpaDProposta.DesEmailSetor;
        VprMensagem.Recipients.Add.Address := VpaDProposta.DesEmailSetor;
        VprMensagem.ReplyTo.EMailAddresses := VpaDProposta.DesEmailSetor;
      end
      else
      begin
      ...
      VprMensagem.Recipients.add.Address := VpaDProposta.DesEmail;

      VpfEmailVendedor := FunCotacao.REmailVencedor(VpaDProposta.CodVendedor);
      ...
      if VpfOpcao <> 0 then
        VprMensagem.Subject := VpaDCliente.NomCliente+ ' - Proposta ' +IntToStr(VpaDProposta.SeqProposta)+' - opção '+IntToStr(VpfOpcao)
      else
        VprMensagem.Subject := VpaDCliente.NomCliente+ ' - Proposta ' +IntToStr(VpaDProposta.SeqProposta);
      result := EnviaEmail(VprMensagem,VprSMTP,VpfDFilial);
      if Result = '' then
      begin
        result := GravaDEmail(VpaDProposta);
        if result = '' then
          result := AlteraEstagioProposta(VpaDProposta.CodFilial,VpaDProposta.SeqProposta,Varia.CodigoUsuario,Varia.EstagioAguardandoRecebimentoProposta,'');
      end;
      VpfEmailHTML.free;
    end;
    VpfOpcoes.free;
  ...
  VpfDFilial.free;
  if result = '' then
    ConcluiAmostraPendenteEnvioEmail(VpaDProposta);
end;
