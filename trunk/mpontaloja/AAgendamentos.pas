...
uses
  ...
  DBTables, Menus, UnClientes, UnDados, Funarquivos, DBClient;
...
uses APrincipal, FunData, FunSql, constantes, ConstMSG, ANovoAgendamento,
  ANovoTeleMarketing, AAlteraEstagioAgendamento;
...
//  Agendamentos.close;
  ...
  Agendamentos.sql.add('select  AGE.CODUSUARIO, AGE.SEQAGENDA, AGE.CODCLIENTE, AGE.DATCADASTRO, '+
                       ...
                       ' AGE.DESOBSERVACAO,AGE.CODUSUARIOAGENDOU, '+
                       ' USU.C_NOM_USU, CLI.C_NOM_CLI, TIP.NOMTIPOAGENDAMENTO, '+
...
var
  VpfDAgenda : TRBDAgenda;
begin
  if AgendamentosSEQAGENDA.AsInteger <> 0 then
  begin
    VpfDAgenda := TRBDAgenda.cria;
