...
uses APrincipal, funString, ConstMsg, AImpCotacao, UnContasAreceber, UnCrystal, Constantes, UnClientes,
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if VpfDProCotacao.QtdConferido > 0 then
          begin
            if (VpfDProCotacao.QtdConferidoSalvo + VpfDProCotacao.QtdConferido) <> (VpfDProCotacao.QtdBaixado) then
      ...
      begin
        ...
        begin
          ...
          begin
            if (VpfDProCotacao.QtdBaixado + VpfDProCotacao.QtdABaixar) < 0  then
              result :=#13+ 'A quantidade a ser estornada do produto "'+VpfDProCotacao.CodProduto+'" é maior que a quantidade que foi baixada.';
          end;
        end
        else
          if VpfDProCotacao.QtdABaixar > (VpfDProCotacao.QtdProduto - VpfDProCotacao.QtdBaixado) then
            result := result + #13+VpfDProCotacao.CodProduto+'-'+VpfDProCotacao.NomProduto;
      end;
    end;
  end;
...
begin
  ...
  if (Varia.CNPJFilial = CNPJ_Metalvidros) then
    GProdutos.Cells[2,GProdutos.ALinha] := FormatFloat('#,###,##0.00',VprDItemCotacao.QtdProduto - VprDItemCotacao.QtdBaixado)
  else
...
begin
  FunCrystal.EnviaEmail(Varia.PathRelatorios+ '\Cotações\XX_Cotação Parcial.rpt',FunClientes.REmailCliente(VprDCotacao.CodCliente),'Pedido Parcial "'+Inttostr(VprDCotacao.LanOrcamento)+'" - ' +Varia.NomeFilial,[IntToStr(VprDCotacao.CodEmpFil),Inttostr(VprDCotacao.LanOrcamento),IntToStr(VprSeqParcial)]);
end;
...
begin
  ...
  begin
    ...
    begin
     ...
     if VprIndConferindo then
     begin
       if VpfDItem.QtdBaixado <= VpfDItem.QtdConferidoSalvo then
         ABrush.Color := clGray;
     end
     else
     begin
       if VpfDItem.QtdProduto <= VpfDItem.QtdBaixado then
         ABrush.Color := clGray;
     end;
    end;
  end;
