...
begin
  VpaTabela.Sql.Clear;
  case CAgruparpor.ItemIndex of
    0 : VpaTabela.Sql.Add('Select Sum(isnull(MOV.N_VLR_PAG,MOV.N_VLR_PAR)) Valor, CAD.D_DAT_EMI DATA1, CAD.D_DAT_EMI DATA ');
    1 : VpaTabela.Sql.Add('Select Sum(MOV.N_VLR_PAR) Valor, (YEAR(CAD.D_DAT_EMI) *100)+month(CAD.D_DAT_EMI) DATA1, month(CAD.D_DAT_EMI)||''/''|| year(CAD.D_DAT_EMI) DATA ');
//    1 : VpaTabela.Sql.Add('Select Sum(isnull(MOV.N_VLR_PAG,MOV.N_VLR_PAR)) Valor, (YEAR(CAD.D_DAT_EMI) *100)+month(CAD.D_DAT_EMI) DATA1, month(CAD.D_DAT_EMI)||''/''|| year(CAD.D_DAT_EMI) DATA ');
    2 : VpaTabela.Sql.Add('Select Sum(isnull(MOV.N_VLR_PAG,MOV.N_VLR_PAR)) Valor,  year(CAD.D_DAT_EMI) DATA1, year(CAD.D_DAT_EMI) DATA ');
  end;
...
begin
  ...
  case CAgruparpor.ItemIndex of
    ...
    1 : VpaTabela.Sql.Add('Select Sum(Orc.N_Vlr_LIQ) Valor, (YEAR(orc.d_dat_orc) *100)+month(orc.d_dat_orc) DATA1, month(Orc.D_DAT_ORC)||''/''|| year(ORC.D_DAT_ORC) DATA, VEN.I_COD_VEN ');
    2 : VpaTabela.Sql.Add('Select Sum(Orc.N_Vlr_LIQ) Valor,  year(ORC.D_DAT_ORC) DATA1, year(ORC.D_DAT_ORC) DATA, VEN.I_COD_VEN ');
  end;
  VpaTabela.Sql.add(' from CadOrcamentos Orc, CADVENDEDORES VEN, CADCLIENTES CLI '+
  ...
  VpaTabela.Sql.Add(' group by DATA1, DATA, VEN.I_COD_VEN '  +
                    ' order by 2,4');
