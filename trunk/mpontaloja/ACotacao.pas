...
type
  TFCotacao = class(TFormularioPermissao)
    ...
    N10: TMenuItem;
    N11: TMenuItem;
    MImprimirPendenciaPedido: TMenuItem;
    BMes: TBitBtn;
    ...
    Rave: TRvProject;
    RvSystem1: TRvSystem;
    rvCadOrcamento: TRvDataSetConnection;
    rvMovOrcamento: TRvDataSetConnection;
    ...
    procedure MImprimirPendenciaPedidoClick(Sender: TObject);
    ...
    procedure CancelarSaldo1Click(Sender: TObject);
  ...
  private
    ...
    procedure VisualizaNotas(VpaLanOrcamento : String);
...
uses APrincipal,Fundata,Constantes, constMsg, ALocalizaProdutos, UnCrystal,
     FunSql, ANovaCotacao,  FunObjeto, AImpCotacao, UnClassesImprimir,
  ...
  ALocalizaClassificacao, dmRave, funNumeros;
...
begin
  ...
  begin
    result := 'FILIAL DA COTAÇÃO INVÁLIDA!!!'#13'Não pode ser gerado a nota fiscal da cotação "'+CadOrcamentoI_Lan_Orc.AsString+'" porque ela não pertence a filial filial ativa.';
    exit(result);
  end;
...
{******************************************************************************}
function TFCotacao.MarcaCotacoesComoFaturadas(VpaLista : TList):String;
var
  VpfLaco : Integer;
  VpfDCotacao : TRBDOrcamento;
begin
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        MarcaCotacoesComoFaturadas(VprCotacoesGerarNota);
      end;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        if varia.CNPJFilial <> CNPJ_CADARTEX THEN
          begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          FunCotacao.ExcluiCotacoesAgrupadas(VprCotacoesGerarNota);
          AtualizaConsulta(false);
        end;
        FNovaCotacao.free;
...
procedure TFCotacao.rvCadOrcamentoValidateRow(Connection: TRvCustomConnection;
  var ValidRow: Boolean);
begin
end;
...
begin
  ...
  begin
    AlterarVisibleDet([BCadastrar,BAlterar,BGraficos,PTotal,BGeraNota,BtExtornar,BRomaneio,BtExcluir,BCancelar,BConsulta,BGeraCupom,BEtiqueta,BGerarOP,BImprimeOP,MBaixarNumero,n7,BCancelar,
                       MImprimirPedidosPedentes,MImprimirPendenciaPedido,MGeraRomaneioParcial,MConferenciaSeparacao,MEnvioParcial,MAlterarCliente,MAlterarVendedor,MAlterarPreposto,MDuplicarOrcamento,
                       MConsultaOP,MMarcarOPGerada],false);
    GOrcamento.Columns[7].Visible := false;
    GridMov.Columns[3].Visible := false;
    GridMov.Columns[7].Visible := false;
    ...
    if (puPLImprimePedPendentes in varia.PermissoesUsuario) then
      AlterarVisibleDet([MImprimirPedidosPedentes,MImprimirPendenciaPedido],true);
    ...
    begin
      ...
      GridMov.Columns[3].Visible := true;
      GridMov.Columns[7].Visible := true;
    end;
...
begin
  VpfPosicao := CadOrcamento.GetBookmark;
   ...
   CadOrcamento.SQl.add(' Select Orc.I_Lan_Orc, Orc.D_Dat_Orc, Cli.C_Nom_Cli,'+
                             ...
                             ' ORC.I_COD_EST, EST.NOMEST, ORC.I_COD_TRA, TRA.C_NOM_TRA, '+
                             ...
                             ' CADTRANSPORTADORAS TRA, CADTIPOORCAMENTO TIP' );
...
//                        ' and ORC.I_COD_TRA *= TRA.I_COD_TRA'+
   ...
   CadOrcamento.sql.add('order by D_DAT_ORC, Orc.I_Lan_Orc');
   CadOrcamento.open;
...
begin
  ...
  begin
   ...
   if varia.CodigoEmpFil <> CadOrcamentoI_EMP_FIL.AsInteger then
     AlterarEnabledDet([BGeraNota,MVisualizarNota,BGeraCupom,BtExtornar,BtExcluir,BAlterar,BGerarOP,BEtiqueta,MEnviarEmail,BCancelar,MGeraRomaneioParcial],false);
...
begin
  ...
  begin
    if (varia.CNPJFilial = CNPJ_Zumm) or
      ...
      FunCrystal.ImprimeRelatorio(Varia.PathRelatorios+ '\Cotações\XX_CotacaoVias.rpt',[CadOrcamentoI_EMP_FIL.AsString,CadOrcamentoI_LAN_ORC.AsString,CadOrcamentoI_EMP_FIL.AsString,CadOrcamentoI_LAN_ORC.AsString])
    else
    begin
  ...
  FunCotacao.SetaOrcamentoImpresso(CadOrcamentoI_EMP_FIL.AsInteger,CadOrcamentoI_lan_orc.AsInteger);
  AtualizaConsulta(True);
end;
...
procedure TFCotacao.VisualizaNotas(VpaLanOrcamento : String);
var
...
{  if CadOrcamentoI_EMP_FIL.AsInteger <> Varia.CodigoEmpFil then
  ...
  begin
    ...
    VpfDContasAReceber := TRBDContasCR.cria;
    if FunCotacao.GeraFinanceiro(VprDOrcamento,nil,VpfDContasAReceber, FunContasAReceber,VpfResultado,true) then
    begin
      if VpfResultado ='' then
      begin
        ...
        if VprDOrcamento.CodOperacaoEstoque <> 0 then
          FunCotacao.BaixaOrcamento(VprDOrcamento);
        if VpfResultado = '' then
...
begin
  if CadOrcamentoI_Lan_Orc.AsInteger <> 0 then
  begin
    if varia.CNPJFilial = CNPJ_Zumm then//grafica zumm
      FunCrystal.ImprimeRelatorio(Varia.PathRelatorios+ '\Ordem Produção\0100PL_OPCotacao.rpt',[CadOrcamentoI_EMP_FIL.AsString,CadOrcamentoI_LAN_ORC.AsString])
    else
    begin
      Rave.close;
...
{******************************************************************************}
procedure TFCotacao.Recibo1Click(Sender: TObject);
var
  VpfDCliente : TRBDCliente;
begin
  if not CadOrcamentoI_Lan_Orc.IsNull then
...
begin
  VisualizaNotas(CadOrcamentoI_Lan_Orc.Asstring);
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if (CadOrcamentoC_NUM_BAI.AsString = 'S') or (CadOrcamentoC_GER_FIN.AsString = 'S') then
      begin
    //    GOrcamento.Canvas.Brush.Color:= clOlive;
...
{******************************************************************************}
procedure TFCotacao.MImprimirPendenciaPedidoClick(Sender: TObject);
begin
  if CadOrcamentoI_Lan_Orc.AsInteger <> 0 then
    FunCrystal.ImprimeRelatorio(Varia.PathRelatorios+ '\Cotações\XX_Cotação Pendente.rpt',[CadOrcamentoI_EMP_FIL.AsString,CadOrcamentoI_LAN_ORC.AsString]);
end;
...
{******************************************************************************}
procedure TFCotacao.CancelarSaldo1Click(Sender: TObject);
var
  VpfResultado : String;
begin
