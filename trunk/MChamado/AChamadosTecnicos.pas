...
uses APrincipal, Constantes, FunData, FunSql, ANovoChamadoTecnico, UnCrystal, ConstMSG,
  AAlteraEstagioChamado, ANovaCotacao, UnCotacao, funObjeto,
...
begin
  ...
  begin
    case ETipoPeriodo.ItemIndex of
      ...
      1 : VpaSelect.add(SQLTextoDataEntreAAAAMMDD('CHA.DATPREVISAO',EDatInicio.Datetime,EDatFim.Datetime,true));
    end;
...
begin
  AlterarEnabledDet([BGerarCotacao,BAlterar],true);
  BGerarCotacao.Enabled := (ChamadoTecnicoCODESTAGIO.AsInteger >= Varia.EstagioChamadoFinalizado);
  if not((puAdministrador in varia.PermissoesUsuario) or (puCHCompleto in varia.PermissoesUsuario)) then
  begin
    if not(puCHAlterarChamadosFinalizados in varia.PermissoesUsuario) and (ChamadoTecnicoCODESTAGIO.Asinteger >= Varia.EstagioChamadoFinalizado) then
...
{******************************************************************************}
procedure TFChamadoTecnico.AlterarEstgio1Click(Sender: TObject);
begin
  if ChamadoTecnicoNUMCHAMADO.AsInteger <> 0 then
  begin
