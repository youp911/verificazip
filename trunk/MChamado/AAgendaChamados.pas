...
type
  TFAgendaChamados = class(TFormularioPermissao)
    ...
    LEMBRETECORPOSEQLEMBRETE: TIntegerField;
    LEMBRETECORPODESTITULO: TStringField;
    LEMBRETECORPOINDLIDO: TStringField;
  ...
  private
    ...
    function RTabelaChamados(VpaPainel : TPanel):TSQLQUERY;
    ...
    procedure AdicionaFiltrosLembrete(VpaSelect: TWideStrings);
...
uses APrincipal, funSql, FunNumeros, AHoraAgendaChamado, Fundata,
  ...
  ANovoLembrete, AVerificaLeituraLembrete;
...
{******************************************************************************}
function TFAgendaChamados.RTabelaChamados(VpaPainel : TPanel):TSQLQUERY;
var
  VpfLaco : Integer;
begin
  for VpfLaco := 0 to VpaPainel.ComponentCount - 1 do
  begin
    if VpaPainel.Components[VpfLaco] is TSQLQUERY then
    begin
      result := TSQLQUERY(VpaPainel.Components[VpfLaco]);
      exit;
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      FunCrystal.ImprimeRelatorioDiretoImpressora(varia.ImpressoraRelatorio,Varia.PathRelatorios + '\Chamados\XX_Chamado.rpt',[VpfTabela.FieldByName('CODFILIAL').AsString,VpfTabela.FieldByName('NUMCHAMADO').AsString],opRetrato);
      VpfTabela.Next;
    end;
  end;
end;
...
{******************************************************************************}
procedure TFAgendaChamados.AdicionaFiltrosLembrete(VpaSelect: TWideStrings);
begin
  ...
  VpaSelect.Add(' AND'+
                ...
                '  OR '+SQLTextoDataEntreAAAAMMDD('LBC.DATAGENDA',EDatInicio.Date,EDatFim.Date,False)+')');
end;

{******************************************************************************}
procedure TFAgendaChamados.Lerlembrete1Click(Sender: TObject);
begin
  if LEMBRETECORPOSEQLEMBRETE.AsInteger <> 0 then
  begin
    FNovoLembrete:= TFNovoLembrete.CriarSDI(Application,'',True);
    FNovoLembrete.LerLembrete(LEMBRETECORPOSEQLEMBRETE.AsInteger,Varia.CodigoUsuario);
    AtualizaLembretes;
    FNovoLembrete.Free;
  end
  else
...
begin
  ...
  begin
    FNovoLembrete:= TFNovoLembrete.CriarSDI(Application,'',True);
    if FNovoLembrete.AlterarLembrete(LEMBRETECORPOSEQLEMBRETE.AsInteger,Varia.CodigoUsuario) then
      AtualizaLembretes;
    FNovoLembrete.Free;
  end
  else
...
begin
  ...
  begin
    ...
    FVerificaLeituraLembrete.VerificaLeitura(LEMBRETECORPOSEQLEMBRETE.AsInteger,Varia.CodigoUsuario);
...
begin
  if LEMBRETECORPOINDLIDO.IsNull then
  begin
