...
uses APrincipal, Constantes,ConstMsg, AProdutosKit,ANovoProdutoPro,
  ALocalizaClassificacao, Unsistema, FunSql;
{$R *.DFM}
...
begin
  ...
  CadProdutos.sql.add(' and Pre.I_Cod_Tab = ' + ETabelaPreco.Text +
                      ...
                      ' AND PRE.I_COD_TAM = QTD.I_COD_TAM');
...
begin
  ...
  begin
    ...
    begin
      ...
      VpaDProdutoPedido.NomCor := CadProdutosCor.AsString;
    end;
    if config.EstoquePorTamanho then
    begin
      VpaDProdutoPedido.CodTamanho := CadProdutosI_COD_TAM.AsInteger;
      VpaDProdutoPedido.NomTamanho := CadProdutosTamanho.AsString;
    end;
  end;
  CadProdutos.close;
end;
...
begin
  if CadProdutosI_COD_COR.AsInteger <> 0 then
    CadProdutosCor.AsString := CadProdutosI_COD_COR.AsString +'-'+ FunProdutos.RNomeCor(CadProdutosI_COD_COR.AsString)
  else
...
{******************************************************************************}
procedure TFlocalizaProduto.BitBtn2Click(Sender: TObject);
begin
  FNovoProdutoPro := TFNovoProdutoPro.CriarSDI(self,'',FPrincipal.VerificaPermisao('FNovoProdutoPro'));
