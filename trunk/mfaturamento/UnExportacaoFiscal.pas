...
Type TRBFuncoesExportacaoFiscal = class
  private
    ...
    function RCodigoFiscalMunicipio(VpaDesCidade : string) : String;
...
begin
  ...
  ' select '+SqlTextoIsNull('CLI.C_CGC_CLI','CLI.C_CPF_CLI')+' CNPJ_CPF, CLI.I_COD_CLI, CLI.C_EST_CLI, C_INS_CLI, FIL.C_EST_FIL, ' +
           ...
           ' CAD.I_COD_PAG, CAD.N_TOT_PRO, CAD.N_TOT_SER, CAD.N_PER_ISQ, CAD.N_VLR_ISQ, '+
...
begin
  AdicionaSQLAbreTabela(Estados,'Select  DISTINCT(C_EST_CLI) ESTADO '+
                                ...
                                ' and CAD.I_EMP_FIL = '+IntToStr(VpaCodfilial)+
                                SQLTextoDataEntreAAAAMMDD('CAD.D_DAT_EMI', VpaDataInicial, VpaDataFinal, True)+
                                ' and CLI.C_EST_CLI <> '''+Varia.EstadoPadrao+'''');
...
begin
  ...
  begin
    ...
    AdicionaCharD(' ',RCodigoFiscalMunicipio(VpaTabela.FieldByName('C_CID_CLI').AsString), 7) +
...
begin
  ...
  while not VpaTabela.Eof do
  begin
    VpfCodFiscalMunicipio := RCodigoFiscalMunicipio(RetiraAcentuacao(VpaTabela.FieldByName('C_CID_CLI').AsString));
...
begin
  ...
  begin
    ...
    RCodigoFiscalMunicipio(VpaTabela.FieldByName('C_CID_CLI').AsString) +'","'+
...
var
  VpfTexto: TStringList;
  VpfAuxStr, VpfContribuinte, VpfTipoCondicaoPgto,VpfNotaCancelada, VpfTipoFrete: string;
  VpfIndNotaCancelada : Boolean;
...
{******************************************************************************}
function TRBFuncoesExportacaoFiscal.RCodigoFiscalMunicipio(VpaDesCidade : string) : String;
begin
  result := '';
