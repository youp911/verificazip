...
//    procedure AlteraQtdReservadaFracaoOPConsumo(VpaDOP : TRBDordemProducao;VpaDFracao : TRBDFracaoOP;VpaDConsumo : TRBDConsumoFracaoOP
  public
    ...
    function ConcluiFracoesSubmontagem(VpaCodFilial, VpaSEqOrdem, VpaSeqFracao : Integer):string;
    ...
    function BaixaOrdemCorte(VpaCodFilial, VpaSeqOrdem, VpaSeqFracao : Integer):string;
    ...
    function ExisteFracaoFilha(VpaCodFilial, VpaSeqOrdem, VpaSeqFracao : Integer) : Boolean;
...
begin
  ...
  AdicionaSQLAbreTabela(OrdCadastro,'Select * from ORDEMPRODUCAOITEM '+
                                    ' Where EMPFIL = 0 AND SEQORD = 0 AND SEQITE = 0' );
...
begin
  ...
  AdicionaSQLAbreTabela(OrdCadastro,'Select * from COLETAOPITEM '+
                                    ' Where EMPFIL = 0 AND SEQORD = 0 AND SEQCOL = 0 AND SEQITE = 0');
...
begin
  ...
  begin
    ...
    if VpfDFracao.IndPossuiEmEstoque then
      OrdCadastro.FieldByName('INDPOSSUIEMESTOQUE').AsString := 'S'
    else
      OrdCadastro.FieldByName('INDPOSSUIEMESTOQUE').AsString := 'N';
...
begin
  ...
  OrdTabela.sql.add('Select FRA.SEQFRACAO, FRA.SEQFRACAOPAI, FRA.SEQPRODUTO, FRA.CODBARRAS, '+
                    ...
                    ' FRA.DESOBSERVACAO, FRA.CODCOR, FRA.CODTAMANHO, FRA.INDPOSSUIEMESTOQUE, '+
  ...
  begin
    ...
    VpfDFracao.IndPossuiEmEstoque := OrdTabela.FieldByName('INDPOSSUIEMESTOQUE').AsString = 'S';
...
begin
  ...
  ExecutaComandoSql(OrdAux,'Delete from ORDEMCORTECORPO'+
                             ' Where CODFILIAL = '+IntToStr(VpaDOrdemCorte.CodFilial)+
                             ' and SEQORDEMPRODUCAO = '+IntTostr(VpaDOrdemCorte.SeqOrdemProducao));
  if VpaDOrdemCorte.Itens.Count > 0  then
  begin
...
begin
  ...
  begin
    if (TRBDFracaoOrdemProducao(VpaListaFracoes.Items[VpfLaco]).SeqFracaoPai = VpaSeqFracaoPaiAtual) and
       (TRBDFracaoOrdemProducao(VpaListaFracoes.Items[VpfLaco]).SeqFracao <> VpaSeqFracaoPaiAtual) then
...
begin
  VpaDEstagio.SeqFracao := VpaDEstagio.SeqFracao;
  ...
  begin
    ...
    if VpaDEstagio.IndEstagioFinal then
    begin
      ConcluiFracoesSubmontagem(VpaDEstagio.CodFilial,VpaDEstagio.SeqOrdem,VpaDEstagio.SeqFracao);
    end;
...
{******************************************************************************}
function TRBFuncoesOrdemProducao.ConcluiFracoesSubmontagem(VpaCodFilial, VpaSEqOrdem, VpaSeqFracao: Integer): string;
var
  VpfTabela : TSQLQuery;
begin
  result := '';
  ...
  begin
    ...
    AdicionaSQLAbreTabela(VpfTabela,'Select CODFILIAL, SEQORDEM, SEQFRACAO, DATFINALIZACAO '+
                                    ' From FRACAOOP '+
                                    ' Where CODFILIAL = '+IntToStr(VpaCodFilial)+
                                    ' AND SEQORDEM = '+IntToStr(VpaSeqOrdem)+
                                    ' AND SEQFRACAOPAI = '+IntToStr(VpaSeqFracao));
    while not VpfTabela.Eof do
    begin
      if VpfTabela.FieldByName('DATFINALIZACAO').AsDateTime < Montadata(1,1,1990) then
      begin
        AlteraEstagioFracaoOP(VpfTabela.FieldByName('CODFILIAL').AsInteger,VpfTabela.FieldByName('SEQORDEM').AsInteger,
                              VpfTabela.FieldByName('SEQFRACAO').AsInteger,varia.EstagioFinalOrdemProducao);
      end;
      ConcluiFracoesSubmontagem(VpfTabela.FieldByName('CODFILIAL').AsInteger,VpfTabela.FieldByName('SEQORDEM').AsInteger,
                              VpfTabela.FieldByName('SEQFRACAO').AsInteger);
      VpfTabela.next;
    end;
    VpfTabela.close;
    VpfTabela.free;
    ...
    begin
      ...
      begin
        if (VpfDProdutoOP.DProduto.QtdRealEstoque > VpfDProdutoOP.QtdOp) then
          ...
          VpfDProdutoOP.QtdaProduzir := VpfDProdutoOP.QtdOp - VpfDProdutoOP.DProduto.QtdRealEstoque;
      end;
...
{******************************************************************************}
function TRBFuncoesOrdemProducao.BaixaOrdemCorte(VpaCodFilial, VpaSeqOrdem, VpaSeqFracao: Integer): string;
begin
  result := '';
  OrdCadastro.Close;
  OrdCadastro.SQL.clear;
  AdicionaSQLTabela(OrdCadastro,'Select * from FRACAOOP ' +
                                    ' Where CODFILIAL = ' +IntToStr(VpaCodFilial)+
                                    ' AND SEQORDEM = ' +IntToStr(VpaSeqOrdem) );
  if VpaSeqFracao <> 0 then
    AdicionaSQLTabela(OrdCadastro,'and SEQFRACAO =  ' + IntToStr(VpaSeqFracao));
  OrdCadastro.open;
  while not OrdCadastro.Eof do
  begin
    OrdCadastro.Edit;
    OrdCadastro.FieldByName('DATCORTE').AsDateTime := now;
    OrdCadastro.Post;
    result := OrdCadastro.AMensagemErroGravacao;
    if OrdCadastro.AErronaGravacao then
      break;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if result = '' then
        begin
          result := GravaConsumoFracoesNovasReimportacao(VpfDNovaOrdemProducao);
        end;
      end;
    end;
...
begin
  ...
  begin
    ...
    VpaDFracaoNova.IndFracaoNova := true;
  end;
end;
...
{******************************************************************************}
function TRBFuncoesOrdemProducao.GravaConsumoFracoesNovasReimportacao(VpaDOrdemProducaoNova: TRBDOrdemProducao): String;
Var
  VpfLaco : Integer;
  VpfDFracao : TRBDFracaoOrdemProducao;
  VpfDOpConsumo : TRBDOrdemProducao;
begin
  result := '';
  //nao pode destruir a VpfDOpConsumo porque senao ira liberar as fracoes da VpaDOrdemProducaoNova da memoria;
  VpfDOpConsumo := TRBDOrdemProducao.cria;
  VpfDOpConsumo.CodEmpresafilial := VpaDOrdemProducaoNova.CodEmpresafilial;
  VpfDOpConsumo.SeqOrdem := VpaDOrdemProducaoNova.SeqOrdem;
  for VpfLaco := 0 to VpaDOrdemProducaoNova.Fracoes.Count - 1 do
  begin
    VpfDFracao := TRBDFracaoOrdemProducao(VpaDOrdemProducaoNova.Fracoes.Items[VpfLaco]);
    if VpfDFracao.IndFracaoNova  then
    begin
      VpfDOpConsumo.Fracoes.add(VpfDFracao);
    end;
  end;
  if VpfDOpConsumo.Fracoes.Count > 0 then
  begin
    result := GravaDConsumoFracoes(VpfDOpConsumo);
    if result = '' then
    begin
      result := GravaDFracaoOPEstagio(VpfDOpConsumo);
    end;
  end;
end;
...
{******************************************************************************}
function TRBFuncoesOrdemProducao.ExisteFracaoFilha(VpaCodFilial, VpaSeqOrdem, VpaSeqFracao: Integer): Boolean;
begin
  AdicionaSQLAbreTabela(OrdAux,'Select COUNT(CODFILIAL) QTD from FRACAOOP '+
                               ' Where CODFILIAL = '+IntToStr(VpaCodFilial)+
                               ' AND SEQORDEM = '+IntToStr(VpaSeqOrdem)+
                               ' AND SEQFRACAOPAI = '+ IntToStr(VpaSeqFracao));
  result := OrdAux.FieldByName('QTD').AsInteger > 0;
  OrdAux.close;
end;
