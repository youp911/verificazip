...
uses
  ...
  ComCtrls, UnOrdemProducao, UnDadosProduto, Menus, ImgList, UnSolicitacaoCompra, UnProdutos,
  CGrades, DBClient;
...
uses APrincipal, FunSql, funData, UnCrystal, Constantes, FunObjeto,
  ...
  AConsultaLogSeparacaoConsumo, dmRave;
...
begin
  ...
  OrdemProducao.Sql.Add('select ORD.EMPFIL, ORD.SEQORD, ORD.DATEMI,  ORD.DATENP, ORD.DATENT,  ORD.NUMPED, '+
                        ...
                        ' FRA.QTDAFATURAR, FRA.QTDREVISADO, '+
                        ' CLI.C_NOM_CLI, ' +
  ...
  if (Varia.TipoOrdemProducao = toSubMontagem) then
    OrdemProducao.sql.add(' and FRA.SEQPRODUTO = PRO.I_SEQ_PRO ')
  else
    ...
    begin
      ...
      begin
        case EPeriodoPor.ItemIndex of
          0 : VpaSelect.Add(SQLTextoDataEntreAAAAMMDD('FRA.DATENTREGA',EDatInicio.DateTime,EDatFim.Datetime,true));
          1 : VpaSelect.Add(SQLTextoDataEntreAAAAMMDD('ORD.DATEMI',EDatInicio.DateTime,EDatFim.Datetime,true));
          2 : VpaSelect.Add(SQLTextoDataEntreAAAAMMDD('FRA.DATFINALIZACAO',EDatInicio.DateTime,EDatFim.Datetime,true));
...
begin
  ...
  begin
    if screen.Height = 768 then
      PanelColor1.Height := 240
    else
      PanelColor1.Height := 241;
    BFiltros.Caption := '<<';
  end
  else
  begin
