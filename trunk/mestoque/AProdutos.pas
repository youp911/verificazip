...
uses APrincipal, fundata, constantes,funObjeto, constMsg, funstring,
     ANovaClassificacao, AImpProduto, FunSql, ANovoProdutoPro;
        ...
        CadClassificacao.SQL.Add('DELETE FROM CADCLASSIFICACAO WHERE I_COD_EMP = ' + IntToStr(varia.CodigoEmpresa) +
         ...
         begin
           ...
           begin
             ...
             ExecutaComandoSql(CadProdutos,' DELETE FROM MOVESTOQUEPRODUTOS WHERE ' +
                                 ' I_SEQ_PRO = ' + IntToStr(TClassificacao(TTreeNode(arvore.Selected).Data).SeqProduto) +
                                 ' AND I_EMP_FIL = ' + IntToStr(Varia.CodigoEmpFil) );

             ExecutaComandoSql(CadProdutos,' DELETE FROM MOVSUMARIZAESTOQUE WHERE ' +
                                 ' I_SEQ_PRO = ' + IntToStr(TClassificacao(TTreeNode(arvore.Selected).Data).SeqProduto) +
                                 ' AND I_EMP_FIL = ' + IntToStr(Varia.CodigoEmpFil) );

             Sistema.GravaLogExclusao('MOVQDADEPRODUTO','Select * from MOVQDADEPRODUTO '+
                                       ' Where I_SEQ_PRO =  '+ IntToStr(TClassificacao(TTreeNode(arvore.Selected).Data).SeqProduto));
             ExecutaComandoSql(CadProdutos,' DELETE FROM MOVQDADEPRODUTO WHERE ' +
                                 ' I_SEQ_PRO = ' + IntToStr(TClassificacao(TTreeNode(arvore.Selected).Data).SeqProduto) );

             ExecutaComandoSql(CadProdutos,' DELETE FROM MOVTABELAPRECO WHERE ' +
                                 ' I_SEQ_PRO = ' + IntToStr(TClassificacao(TTreeNode(arvore.Selected).Data).SeqProduto) );

             Sistema.GravaLogExclusao('MOVKIT','Select * from MOVKIT '+
                                       ' Where I_PRO_KIT =  ' +IntToStr(TClassificacao(TTreeNode(arvore.Selected).Data).SeqProduto));
             ExecutaComandoSql(CadProdutos,'DELETE FROM MOVKIT '+
                                           ' Where I_PRO_KIT = '+ IntToStr(TClassificacao(TTreeNode(arvore.Selected).Data).SeqProduto));
             ExecutaComandoSql(CadProdutos,'DELETE FROM BRINDECLIENTE '+
                                           ' Where SEQPRODUTO = '+ IntToStr(TClassificacao(TTreeNode(arvore.Selected).Data).SeqProduto));

             Sistema.GravaLogExclusao('CADPRODUTOS','Select * from CADPRODUTOS '+
                                       ' Where I_SEQ_PRO =  ' +IntToStr(TClassificacao(TTreeNode(arvore.Selected).Data).SeqProduto));
             ExecutaComandoSql(CadProdutos,' DELETE FROM CADPRODUTOS WHERE ' +
                                 ' I_SEQ_PRO = ' + IntToStr(TClassificacao(TTreeNode(arvore.Selected).Data).SeqProduto) );
