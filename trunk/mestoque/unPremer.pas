...
Type TRBFuncoesPremer = class(TRBLocalizaPremer)
  private
    ...
    function AssociaEstagiosProduto(VpaSeqProduto : Integer;VpaCodProduto : String) : string;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        aviso('ERRO NA IMPORTA플O DO PRODUTO "'+VpaCodProduto+'"');
        VpfSigProduto := '90';
      end;
...
begin
  ...
  VpaLinha := DeleteAteChar(DeleteAteChar(DeleteAteChar(DeleteAteChar(DeleteAteChar(VpaLinha,';'),';'),';'),';'),';');
  ...
  try
    ...
    on e : exception do result := 'ERRO NA GRAVA플O DO CONSUMO DO PRODUTO!!!'+#13+e.message;
  end;
  Cadastro.close;
end;
...
begin
  ...
  try
    ...
    on e : exception do result := 'ERRO NA GRAVA플O DO ESTAGIO DO PRODUTO!!!'+#13+e.message;
  end;
  Cadastro.close;
end;
...
begin
  ...
  begin
    ...
    if result = '' then
      result := AssociaEstagiosProduto(VpaDProduto.SeqProduto,VpaDProduto.CodProduto);
  end;
end;
...
begin
  ...
  begin
    ...
    if Cadastro.AErronaGravacao then
      result := 'ERRO NA GRAVA플O DO CONSUMO DO PRODUTO!!!'+#13+Cadastro.AMensagemErroGravacao;
    Cadastro.close;
  end;
end;

{******************************************************************************}
function TRBFuncoesPremer.AssociaEstagiosProduto(VpaSeqProduto : Integer;VpaCodProduto : String) : string;
begin
  result := '';
