...
Type TRBFuncoesClientes = class(TRBLocalizaClientes)
  private
    ...
    function ExisteProdutoCliente(VpaNumSerie : string):boolean;
    ...
    function CadastraProdutoNumeroSerieCliente(VpaDatEmissao : TDateTime; VpaCodCliente, VpaSeqProduto,VpaQtdMesesGarantia : Integer;VpaNumSerie, VpaUM, VpaNomProduto : String):string;
    ...
    function CarDTelemarketingProspect(VpaDProspect : TRBDDigitacaoProspectItem):TRBDTelemarketing;
    function CarDTelemarketingSuspect(VpaDProspect : TRBDDigitacaoProspectItem):TRBDTelemarketingProspect;
  public
    ...
    function GravaDAgenda(VpaDAgenda : TRBDAgenda): String;
    ...
    function AlteraEstagioAgenda(VpaDAgenda : TRBDAgenda;VpaCodEstagio : Integer) : string;
...
Uses FunSql, Constantes, ConstMsg, FunString, FunValida, FunData, FunObjeto, UnProdutos,
     UnProspect, UnContasAReceber;
...
{******************************************************************************}
function TRBFuncoesClientes.ExisteProdutoCliente(VpaNumSerie : string):boolean;
begin
...
begin
  ...
  CliTabela.sql.add('select I_COD_VEN, COUNT(I_COD_CLI) QTD from CADCLIENTES CLI '+
                                  ' WHERE (CLI.C_IND_CLI = ''S'''+
                                  ' or CLI.C_IND_PRC = ''S'')');
  if VpaDClientePerdido.QtdDiasSemPedido <> 0 then
    CliTabela.sql.add(' and datediff(day, isnull(d_dat_cad,''2000/01/01''),'+SQLTextoDataAAAAMMMDD(date)+')  > '+InttoStr(VpaDClientePerdido.QtdDiasSemPedido)+
  ...
  if VpaDClientePerdido.QtdDiasComPedido <> 0 then
    CliTabela.sql.add(' and datediff(day, isnull(d_dat_cad,''2000/01/01''),'+SQLTextoDataAAAAMMMDD(date)+')  > '+InttoStr(VpaDClientePerdido.QtdDiasComPedido)+
  ...
  begin
    ...
    try
      ...
      on e : exception do result := 'ERRO NA GRAVAÇÃO DO CLIENTEPERDIDOVENDEDORITEM '#13+e.message;
    end;
    if result <> '' then
      CliTabela.last;
    inc(VpfItem);
...
begin
  ...
  cliCadastro.FieldByName('C_COB_FRM').AsString := 'S';
  ...
  try
    ...
    on e : exception do result := 'ERRO NA GRAVAÇÃO DO PROSPECT!!!'#13+e.message;
  end;
  CliCadastro.Close;
end;
...
begin
  ...
  CliAux.sql.add('UPDATE CADCLIENTES CLI '+
                   ...
                   ' WHERE (CLI.C_IND_CLI = ''S'''+
                                  ' or CLI.C_IND_PRC = ''S'')');
  if VpaDClientePerdido.QtdDiasSemPedido <> 0 then
    CliAux.sql.add(' and datediff(day, isnull(d_dat_cad,''2000/01/01''),'+SQLTextoDataAAAAMMMDD(date)+')  > '+InttoStr(VpaDClientePerdido.QtdDiasSemPedido)+
                      ' and NOT EXISTS(select * from CADORCAMENTOS ORC '+
                      '       Where ORC.D_DAT_ORC >= '+SQLTextoDataAAAAMMMDD(DecDia(Date,VpaDClientePerdido.QtdDiasSemPedido))+
                      '       AND ORC.I_COD_CLI = CLI.I_COD_CLI) ');
  if VpaDClientePerdido.QtdDiasComPedido <> 0 then
    CliAux.sql.add(' and datediff(day, isnull(d_dat_cad,''2000/01/01''),'+SQLTextoDataAAAAMMMDD(date)+')  > '+InttoStr(VpaDClientePerdido.QtdDiasComPedido)+
                      ' and EXISTS(select * from CADORCAMENTOS ORC '+
                      '       Where ORC.D_DAT_ORC >= '+SQLTextoDataAAAAMMMDD(DecDia(Date,VpaDClientePerdido.QtdDiasComPedido))+
                      '       AND ORC.I_COD_CLI = CLI.I_COD_CLI) ');
  if VpaDClientePerdido.CodRegiaoVendas <> 0 then
...
{******************************************************************************}
function TRBFuncoesClientes.CadastraProdutoNumeroSerieCliente(VpaDatEmissao : TDateTime; VpaCodCliente, VpaSeqProduto,VpaQtdMesesGarantia : Integer;VpaNumSerie, VpaUM, VpaNomProduto : String ):string;
var
  VpfProdutos : TList;
  VpfDProdutoCliente : TRBDProdutoCliente;
begin
  result := '';
  if not ExisteProdutoCliente(VpaNumSerie) then
  begin
...
{******************************************************************************}
function TRBFuncoesClientes.CarDTelemarketingProspect(VpaDProspect : TRBDDigitacaoProspectItem):TRBDTelemarketing;
begin
  ...
  result.CodHistorico := varia.CodHistoricoLigacaoVisitaVendedor;
  result.DesObservacao := VpaDProspect.DesHistorico;
  result.DatLigacao := VpaDProspect.DatVisita;
end;

{******************************************************************************}
function TRBFuncoesClientes.CarDTelemarketingSuspect(VpaDProspect : TRBDDigitacaoProspectItem):TRBDTelemarketingProspect;
begin
...
begin
  AdicionaSqlAbreTabela(CliAux,'Select * from CADTRANSPORTADORAS '+
                              ' Where I_COD_TRA = '+Inttostr(VpaCodtransportadora));
  result := CliAux.FieldByName('C_NOM_TRA').AsString;
  CliAux.Close;
end;
...
begin
  ...
  begin
    ...
    ExecutaComandoSql(CliAux,'UPDATE PRODUTOFORNECEDOR '+
                             ' SET CODCLIENTE = '+IntToStr(VpaCodClienteHistorico)+
                             ' where CODCLIENTE = '+IntToStr(VpaCodClienteAExcluir));
...
begin
  ...
  begin
    ...
    try
      ...
      on e : exception do result := 'ERRO NA GRAVAÇÃO DOS PRODUTOS DO CLIENTE!!!'#13+e.message;
    end;
  end;
  CliCadastro.close;
end;
...
{******************************************************************************}
function TRBFuncoesClientes.GravaDAgenda(VpaDAgenda : TRBDAgenda): String;
begin
  result := '';
...
begin
  ...
  begin
    ...
    begin
      if VpfDProspect.DesTipo = 'P' then
      begin
        VpfDTelemarketing := CarDTelemarketingProspect(VpfDProspect);
        ...
        if VpfDProspect.DesTipo = 'S' then
        begin
          VpfDTeleMarketingSuspect := CarDTelemarketingSuspect(VpfDProspect);
...
begin
  ...
  begin
    ...
    CliCadastro.FieldByName('TIPCREDITO').AsString := VpfDCreditoCliente.TipCredito;
...
{******************************************************************************}
function TRBFuncoesClientes.AlteraEstagioAgenda(VpaDAgenda : TRBDAgenda;VpaCodEstagio : Integer) : string;
begin
  result := '';
  AdicionaSQLAbreTabela(CliCadastro,'Select * from AGENDA '+
                                    ' Where CODUSUARIO = '+IntToStr(VpaDAgenda.CodUsuario)+
...
{******************************************************************************}
procedure TRBFuncoesClientes.CarDAgenda(VpaCodUsuario,VpaSeqAgenda : Integer;VpaDAgenda : TRBDAgenda);
begin
...
begin
  ...
  begin
    ...
    VpfDCredito.TipCredito := CliTabela.FieldByName('TIPCREDITO').AsString;
...
begin
  ...
  CliCadastro.Insert;
  CliCadastro.FieldByname('CODCLIENTE').AsInteger := VpaCodCliente;
  CliCadastro.FieldByname('DESEMAIL').AsString := VpaDesEmail;
...
begin
  ...
  try
    ...
    on e : exception do result := 'ERRO NA GRAVAÇÃO DO CLIENTEPERDIDOVENDEDOR!!!'#13+e.message;
  end;
  if result = '' then
  begin
    ...
    begin
      ...
      begin
        result := CadastraProdutoNumeroSerieCliente(VpaDCotacao.DatOrcamento,VpaDCotacao.CodCliente,VpfDProdutoOrc.SeqProduto,
                        VpfDProdutoOrc.QtdMesesGarantia,VpfDProdutoOrc.DesRefCliente,VpfDProdutoOrc.UM,VpfDProdutoOrc.NomProduto);
        if result <> '' then
          break;
      end;
    end;
    ...
    begin
      ...
      begin
        result := CadastraProdutoNumeroSerieCliente(VpaDNota.DatEmissao,VpaDNota.CodCliente,VpfDProdutoNota.SeqProduto,
                        VpfDProdutoNota.QtdMesesGarantia,VpfDProdutoNota.DesRefCliente,VpfDProdutoNota.UM,VpfDProdutoNota.NomProduto);
        if result <> '' then
          break;
      end;
    end;
