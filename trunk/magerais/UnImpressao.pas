...
begin
  ...
  ConfiguraImp := TConfiguraImp.Criar(Aowner,VpaBaseDados);
  Itens := TSQLQuery.Create(Aowner);
  ...
  ClientesEmAberto := TSQLQUERY.Create(Aowner);
...
begin
  ...
  FunNotaFiscal.CarDTransportadora(VprDTransportadora,VpaDCotacao.CodTransportadora);
...
begin
  ...
  begin
    ...
    FunClientes.CarDCliente(VprDCliente);
    FunNotaFiscal.CarDTransportadora(VprDTransportadora,VpaDNotaFiscal.CodTransportadora);
...
begin
  ...
  case VpaDItemImpressao.SeqDocumento of
    ...
    16 : result := VprDTransportadora.Nome ;
    17 : result := VprDCotacao.PlaVeiculo;
    18 : result := VprDCotacao.UFVeiculo;
    19 : result := VprDTransportadora.CGC_CPF;
    20 : result := VprDTransportadora.Endereco+', '+VprDTransportadora.NroEndereco;
    21 : result := VprDTransportadora.Cidade;
    22 : result := VprDTransportadora.UF;
    ...
    33 : begin
           if VpaDImpressao.IndiceItem < VprDCotacao.Produtos.Count then
           begin
            if (Varia.CNPJFilial = CNPJ_REELTEX)and (VprDImpressao.DesTipoDocumento = 'PED') Then
              result :=  FormatFloat(varia.MascaraQTD,TRBDOrcProduto(VprDCotacao.Produtos.Items[VpaDImpressao.IndiceItem]).QtdProduto - TRBDOrcProduto(VprDCotacao.Produtos.Items[VpaDImpressao.IndiceItem]).QtdBaixado)
            else
...
{           if ((varia.CNPJFilial = '81.011.082/0001-71') or(varia.CNPJFilial = '00.539.911/0001-91'))and (Varia.NotaFiscalPadrao = 2) then
    ...
    31 : result := VprDTransportadora.Nome;
    ...
    36 : result := VprDTransportadora.Endereco;
    37 : result := VprDTransportadora.Cidade;
    38 : result := VprDTransportadora.UF;
    ...
    46 : result := FunNotaFiscal.RetornaCondPagamento(VprDNotaFiscal.CodCondicaoPagamento);
