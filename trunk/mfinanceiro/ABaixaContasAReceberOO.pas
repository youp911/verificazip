...
type
  TFBaixaContasaReceberOO = class(TFormularioPermissao)
    ...
    procedure BOpcoesClick(Sender: TObject);
  private
    VprAcao: Boolean;
...
procedure TFBaixaContasaReceberOO.BOpcoesClick(Sender: TObject);
begin
...
begin
  result := '';
  if (VprDBaixaCR.TipFormaPagamento = 'C') or (VprDBaixaCR.TipFormaPagamento = 'R') or
     (VprDBaixaCR.TipFormaPagamento = 'A') or (VprDBaixaCR.TipFormaPagamento = 'P') then // cheque, cheque terceiros, carteira, Deposito Cheque
  begin
...
procedure TFBaixaContasaReceberOO.EFormaPagamentoRetorno(Retorno1,
  Retorno2: String);
begin
  VprDBaixaCR.TipFormaPagamento := Retorno1;
end;
