...
begin
  ...
  begin
   ...
   // filtro empresa / filial
    ...
    if EPlano.Text <> '' then
      VpaSelect.add(' and CP.C_CLA_PLA like ''' + Trim(EPlano.Text) + '%''' );
...
begin
  FImprimeCP := TFImprimeCP.Create(application);
  FImprimeCP.ImprimeContasAPagarPlanoContas(ECodFilial.Text, ECodCliente.text,EPlano.Text,SitPar.ItemIndex,RPeriodo.ItemIndex, dataParcela1.Date,dataParcela2.Date,LFilial.Caption,LPlano.Caption,Label20.Caption,SitPar.Items.Strings[SitPar.itemIndex],RPeriodo.Items.Strings[SitPar.itemIndex]);
  FImprimeCP.free;

{  FImprimeCP.CarregaImpressao(MovParcelas.Text,dataParcela1.Date,dataParcela2.Date, varia.NomeEmpresa,
                               , '',
                              Label20.Caption, SitPar.Items.Strings[SitPar.itemIndex],numerico2.Text,numerico1.Text);
...
begin
  ...
  begin
    if screen.Height = 768 then
      PanelColor3.Height := 143
    else
      PanelColor3.Height := 112;
    BFiltros.Caption := '>>';
  end;
