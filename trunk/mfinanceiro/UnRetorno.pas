...
type
  TRBDLocalizacaoRetorno = class
    public
      ...
      procedure PosNossoNumero(VpaTabela : TDataSet;VpaNossoNumero : string);
  end;
  ...
  TRBDFuncoesRetorno = class(TRBDLocalizacaoRetorno)
    private
      ...
      procedure CarLanReceberItem(VpaDItem : TRBDRetornoItem);
...
{******************************************************************************}
procedure TRBDLocalizacaoRetorno.PosNossoNumero(VpaTabela : TDataSet;VpaNossoNumero : string);
begin
  AdicionaSqlAbreTabela(VpaTabela,'Select * from MOVCONTASARECEBER '+
...
{******************************************************************************}
procedure TRBDFuncoesRetorno.CarLanReceberItem(VpaDItem : TRBDRetornoItem);
var
...
begin
  ...
  begin
    if VpaDItem.DesNossoNumero <> '' then
      PosNossoNumero(Aux,VpaDItem.DesNossoNumero)
    else
    begin
...
begin
    ...
    VpaDItemRetorno.DesNossoNumero := DeletaCharE(DeletaCharD(copy(vpfLinha,38,20),' '),'0');
...
begin
  ...
  if (copy(VpfArquivo.Strings[0],77,3) ='341') then //itau
