...
type
  ...
  public
    ...
    function SomaValorReceber(VpaOrdem, VpaParcela: Integer): Double;
end;
...
Type
  ...
  public
    ...
    procedure LocalizaParcelasSemParciais(tabela : TDataSet; lancamento : Integer);
    procedure LocalizaParcelasComParciais(VpaTabela : TDataSet; VpaCodFilial, VpaLanReceber: Integer);
    procedure LocalizaParcelasFRM(tabela : TDataSet; lancamento : Integer);
    ...
    procedure LocalizaJoinContaParcelas(tabela : TDataSet; lancamento : Integer; CampoOrderBy : string);
    procedure LocalizaMov(VpaTabela : TDataSet);
    procedure LocalizaParcelasAbertas(Vpatabela : TDataSet;  VpaLanReceber : Integer; VpaCampoOrderBy : string);
    procedure LocalizaParcelaAberta(VpaTabela : TDataSet; VpaLanReceber, VpaNumParcela : Integer );
...
type
  ...
  private
    ...
    procedure CarAgenciaBancodaContaCorrente(VpaNumConta : String;Var VpaCodBanco : Integer;var VpaNumAgencia : string);
  ...
  public
    ...
    Function RetornaProximoCodigo(CampoFilial, Campo, NomeTabela : String):Integer;
    ...
    function  GeraNumeroDuplicata( NroNotaFiscal : Integer) : integer;
    ...
    function RValTotalCredito(VpaCreditos : TList) : double;
    function ClientePossuiCredito(VpaCreditos : TList) : Boolean;
    ...
    // exclusao e estorno
    function RetornaValorTotalParcelas(Lancamento : Integer): Double;
...
function TCalculosContasAReceber.SomaValorReceber(VpaOrdem, VpaParcela: Integer): Double;
begin
  AdicionaSQLAbreTabela(calcula,
...
procedure TLocalizaContasAReceber.LocalizaParcelasSemParciais(tabela : TDataSet; lancamento : Integer);
begin
...
{*********************** localiza parcelas sem parciais ********************** }
procedure TLocalizaContasAReceber.LocalizaParcelasFRM(tabela : TDataSet; lancamento : Integer);
begin
  AdicionaSQLAbreTabela(tabela,
    ' select * from MovContasaReceber as Mov, CadFormasPagamento as Pag' +
    ' where MOV.I_LAN_REC = ' + IntToStr(Lancamento) +
...
{******************************************************************************}
procedure TLocalizaContasAReceber.LocalizaJoinContaParcelas(tabela : TDataSet; lancamento : Integer; CampoOrderBy : string);
begin
  AdicionaSQLAbreTabela(Tabela,
    ...
    ' and CR.I_LAN_REC = ' + IntToStr(lancamento) +
...
{******************************************************************************}
procedure TLocalizaContasAReceber.LocalizaParcelasAbertas(VpaTabela : TDataSet; VpaLanReceber : Integer; VpaCampoOrderBy : string);
begin
  AdicionaSQLAbreTabela(VpaTabela,    ' select * from MOVCONTASARECEBER ' +
    ' where I_EMP_FIL = ' + IntTostr(varia.CodigoEmpFil) +
    ' and I_LAN_REC = ' + IntToStr(VpaLanReceber) +
...
{******************************************************************************}
procedure TLocalizaContasAReceber.LocalizaParcelaAberta(VpaTabela : TDataSet; VpaLanReceber, VpaNumParcela : Integer );
begin
  AdicionaSQLAbreTabela(Vpatabela,' select * from MovContasAReceber ' +
    ' where I_EMP_FIL = ' + IntTostr(varia.CodigoEmpFil) +
    ' and I_LAN_REC = ' + IntToStr(VpaLanReceber) +
...
begin
  ...
  begin
    ...
    VpfDParcela.DatProrrogacao := VpfDatVencimento;
    ...
    if VpfDFormaPagamento.FlaTipo = 'B' then
    begin
      ...
      CarAgenciaBancodaContaCorrente(VpfDParcela.NroContaBoleto,VpfDParcela.CodBanco,VpfDParcela.NroAgencia);
    end;
...
begin
  ...
  begin
    ...
    CadContas.FieldByName('D_ULT_ALT').AsDateTime := Date;
...
begin
  ...
  if (ConfigModulos.Comissao)  then
    ...
    begin
      ...
      if VpaDNovaCR.TipComissao = 0 then
...
begin
  ...
  CadContas.FieldByName('C_BAI_CAR').AsString := 'N';
  CadContas.FieldByName('D_ULT_ALT').AsDateTime := now;
  CadContas.Post;
  result := CadContas.AMensagemErroGravacao;
  CadContas.close;
end;
...
{******************************************************************************}
procedure TFuncoesContasAReceber.CarAgenciaBancodaContaCorrente(VpaNumConta : String;Var VpaCodBanco : Integer;var VpaNumAgencia : string);
begin
  AdicionaSQLAbreTabela(Aux,'Select * from CADCONTAS '+
                            ' Where C_NRO_CON = '''+VpaNumConta+'''');
...
begin
  ...
  VpaDFormaPagamento.CodForma := Aux.FieldByName('I_COD_FRM').AsInteger;
  VpaDFormaPagamento.NomForma := Aux.FieldByName('C_NOM_FRM').AsString;
  VpaDFormaPagamento.FlaTipo := Aux.FieldByName('C_FLA_TIP').AsString[1];
  VpaDFormaPagamento.IndBaixarConta := (UpperCase(Aux.FieldByName('C_BAI_CON').AsString) = 'S');
  Aux.Close;
end;
...
Function TFuncoesContasAReceber.RetornaProximoCodigo(CampoFilial, Campo, NomeTabela : String):Integer;
begin
   AdicionaSQLAbreTabela(Tabela,'Select max('+ campo + ')' + campo + '  from ' + NomeTabela +
                                ' Where '+ CampoFilial +' = ' + IntToStr(varia.CodigoEmpFil) );         //Codigo da filial Ativa);
   result := Tabela.FieldByName(Campo).AsInteger + 1;
   Tabela.close;
end;
...
begin
  VpaDNovaCR.LanReceber := GeraProximoCodigo('i_lan_rec','CadContasaReceber','I_emp_Fil', VpaDNovaCR.CodEmpFil,  false,Cadastro.ASqlConnection);
...
function TFuncoesContasAReceber.GeraNumeroDuplicata( NroNotaFiscal : Integer) : integer;
begin
...
function TFuncoesContasAReceber.VerificaAtualizaValoresParcelas(ValorInicialParcelas : double; lancamento : Integer ) : boolean;
var
  Total : double;
begin
...
{******************************************************************************}
function TFuncoesContasAReceber.RValTotalCredito(VpaCreditos : TList) : double;
var
  VpfLaco : Integer;
begin
  result := 0;
  ...
  begin
    if not TRBDCreditoCliente(VpaCreditos.Items[VpfLaco]).IndFinalizado and
       (TRBDCreditoCliente(VpaCreditos.Items[VpfLaco]).TipCredito = 'C') then
      result := result + TRBDCreditoCliente(VpaCreditos.Items[VpfLaco]).ValCredito;
  end;
end;

{******************************************************************************}
function TFuncoesContasAReceber.ClientePossuiCredito(VpaCreditos : TList) : Boolean;
var
  VpfLaco : Integer;
begin
  result := false;
  ...
  begin
    if (TRBDCreditoCliente(VpaCreditos.Items[VpfLaco]).ValCredito <> 0) and
       (TRBDCreditoCliente(VpaCreditos.Items[VpfLaco]).TipCredito = 'C') and
...
begin
  ...
  Cadastro.FieldByName('D_DAT_VEN').AsDateTime := VpaDatVencimento;
...
begin
  ...
  if (VpaDParcela.NumDiasAtraso > 0)and (VpaDParcela.NumDiasAtraso > VpaDParcela.NumDiasCarencia) then
  begin
    if Config.BaixaContasReceberCalcularJuros then
    begin
      if VpaDParcela.PerMora <> 0 then
        VpaDParcela.ValAcrescimo := ((((VpaDParcela.ValParcela * VpaDParcela.PerMora) / 100 ) * VpaDParcela.NumDiasAtraso)/30)
      else
        if VpaDParcela.PerJuros <> 0 then
          VpaDParcela.ValAcrescimo := ((((VpaDParcela.ValParcela * VpaDParcela.PerJuros) / 100 ) * VpaDParcela.NumDiasAtraso)/30)
    end;
  end;
...
function TFuncoesContasAReceber.RetornaValorTotalParcelas(Lancamento : Integer): Double;
begin
  AdicionaSQLAbreTabela( Tabela,' select * from CadContasaReceber ' +
    ' where I_LAN_REC = ' + IntToStr(lancamento) +
    ' and I_EMP_FIL = ' + IntToStr(varia.CodigoEmpFil));
...
begin
  ...
  begin
    ...
    VpfDForma.CodForma := Tabela.FieldByName('I_COD_FRM').AsInteger;
    VpfDForma.NomForma := Tabela.FieldByName('C_NOM_FRM').AsString;
    VpfDForma.FlaTipo := Tabela.FieldByName('C_FLA_TIP').AsString[1];
    VpfDForma.IndBaixarConta := (UpperCase(Tabela.FieldByName('C_BAI_CON').AsString) = 'S');
    Tabela.Next;
  end;
  Tabela.close;
end;
...
begin
  ...
  begin
    ...
    begin
      VpfDCheque := TRBDCheque(VpaCheques.Items[VpfLaco]);
      if (VpfDCheque.TipFormaPagamento <> 'C') and
         (VpfDCheque.TipFormaPagamento <> 'R') and
         (VpfDCheque.TipFormaPagamento <> 'P') then //cheque e cheque de terceiros.
         VpfDCheque.DatCompensacao := date;
      if (VpfDCheque.TipCheque = 'C') AND
         (VpfDCheque.TipContaCaixa = 'CC') and
         (VpfDCheque.TipFormaPagamento <> 'P') then
         VpfDCheque.DatCompensacao := date;
...
begin
  ...
  begin
    ...
    VpaDCheque.TipFormaPagamento := Aux.FieldByname('C_FLA_TIP').AsString;
  end;
  Aux.Close;
end;
