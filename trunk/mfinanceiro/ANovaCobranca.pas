...
uses
  ...
  DBKeyViolation, ComCtrls, Localizacao, Menus, UnDadosProduto, UnCrystal,
...
uses APrincipal, FunData, FunSql, ConstMsg, AHistoricoLigacao, UnContasaReceber, funObjeto,
  ...
  AImpCotacao, AMotivoInadimplencia, FunArquivos,
...
begin
  ...
  begin
    ...
    if varia.CNPJFilial = CNPJ_RELOPONTO then
      Ligacoes.sql.add('AND CAD.I_EMP_FIL = 11 ');
...
begin
  ...
  if Entrada('Texto de referencia do recibo','Referente : ',vpfDescricao,false,ECodCliente.color,PanelColor2.color) then
    ...
    begin
      ...
      dtRave.ImprimeRecibo(varia.CodigoEmpFil,VprDCliente,VpfDescricao,FormatFloat('#,###,##0.00',StrToFloat(VpfValor)),Extenso(StrToFloat(VpfValor),'real','reais'),varia.CidadeFilial+' '+ IntTostr(dia(date))+', de ' + TextoMes(date,false)+ ' de '+Inttostr(ano(date)));
      dtRave.free;
    end;
end;
...
begin
  ...
  CADORCAMENTO.SQL.Add('Select Orc.I_Lan_Orc, Orc.D_Dat_Orc, Cli.C_Nom_Cli, Pag.C_Nom_Pag,'+
                       ...
                       ' ESTAGIOPRODUCAO EST, CADTRANSPORTADORAS TRA,'+
