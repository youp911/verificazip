...
uses
  ...
  ComCtrls, Mask, numericos, Graficos,Series,TeeProcs,TeEngine;
...
type
  ...
  private
    ...
    procedure PosPrazoRecebimento(VpaTabela : TQuery);
...
{******************************************************************************}
procedure TFPrazoMedio.PosPrazoRecebimento(VpaTabela : TQuery);
begin
  VpaTabela.sql.clear;
  case CAgruparpor.ItemIndex of
    ...
    1 : VpaTabela.Sql.Add('select SUM((MOV.D_DAT_VEN - CAD.D_DAT_EMI)) / COUNT(CAD.I_EMP_FIL) PRAZO, (YEAR(CAD.D_DAT_EMI) *100)+month(CAD.D_DAT_EMI)  DATA1, month(CAD.D_DAT_EMI)||''/''|| year(CAD.D_DAT_EMI) DATA');
    2 : VpaTabela.Sql.Add('select SUM((MOV.D_DAT_VEN - CAD.D_DAT_EMI)) / COUNT(CAD.I_EMP_FIL) PRAZO, year(CAD.D_DAT_EMI) DATA1, year(CAD.D_DAT_EMI) DATA');
  end;
